...
namespace JudgeJanken.iOS {
	...
	/// </summary>
	public class jankenmodelInput : NSObject, IMLFeatureProvider
	{
		...
		public jankenmodelInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class jankenmodelOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public jankenmodelOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class jankenmodel : NSObject
	{
		readonly MLModel model;

		static NSUrl GetModelUrl ()
		{
			return NSBundle.MainBundle.GetUrlForResource ("jankenmodel", "mlmodelc");
		}
		...
		jankenmodel (MLModel model)
		{
			this.model = model;
		}

		public static jankenmodel Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new jankenmodel (model);
		}

		public static jankenmodel Create (MLModelConfiguration configuration, out NSError error)
		{
		...
		public static jankenmodel Create (NSUrl url, MLModelConfiguration configuration, out NSError error)
		{
			if (url == null)
				throw new ArgumentNullException (nameof (url));
		...
		/// </summary>
		/// <param name="input">an instance of jankenmodelInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public jankenmodelOutput GetPrediction (jankenmodelInput input, out NSError error)
		{
			if (input == null)
			...
			return new jankenmodelOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public jankenmodelOutput GetPrediction (jankenmodelInput input, MLPredictionOptions options, out NSError error)
		{
			if (input == null)
				throw new ArgumentNullException (nameof (input));
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public jankenmodelOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
